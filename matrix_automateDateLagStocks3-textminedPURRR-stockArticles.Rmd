...
!@#$% We are going to extend this tutorial, to show textmining of those webarticles of stocks with the PURRR package. The source for this part of the textmined articles is in the [tidytext documentation](https://www.tidytextmining.com/dtm.html)
```{r}
...
```{r}
write.csv(portfolio, paste(paste(paste('portfolio',date, sep='_'),
                           today,sep='_'),
                           'csv',sep='.'),
          row.names=TRUE)
#if yahoo finance or code chunk above doesn't work read in last portfolio:

#portfolio <- read.csv('portfolio_1999-01-01_2020-03-15.csv',
...
inCompREQ_stocks <- merge(inCompREQ_stocks,stockNames, by.x='stockName',by.y='stock')
inCompREQ_stocks <- inCompREQ_stocks[,c(1,24,25,2:7,20:23,8:19)]
```

```{r}
write.csv(inCompREQ_stocks, paste(paste(paste(paste('incomplete_stocks',date,sep='_'),
                                        today,sep='_'),lagN, sep='_'), 
...
a <- colSums(is.na(Close_noNAs))
CloseAll <- Close_noNAs[,a==0]
```
...
df52 <- df52[complete.cases(df52),]

Laal <- subset(df52, df52$stockName=='AAL')
Laal$startDayValue <-Laal$stockDayValue[1]
Laal$startDayDate <- Laal$Date[1]
Laal$finalDayValue <-Laal$stockDayValue[length(Laal$stockDayValue)]
Laal$finalDayDate <- Laal$Date[length(Laal$Date)]

Laap <- subset(df52, df52$stockName=='AAP')
Laap$startDayValue <-Laap$stockDayValue[1]
Laap$startDayDate <- Laap$Date[1]
Laap$finalDayValue <-Laap$stockDayValue[length(Laap$stockDayValue)]
Laap$finalDayDate <- Laap$Date[length(Laap$Date)]

Laddyy <- subset(df52, df52$stockName=='ADDYY')
Laddyy$startDayValue <-Laddyy$stockDayValue[1]
Laddyy$startDayDate <- Laddyy$Date[1]
Laddyy$finalDayValue <-Laddyy$stockDayValue[length(Laddyy$stockDayValue)]
Laddyy$finalDayDate <- Laddyy$Date[length(Laddyy$Date)]

Lamzn <- subset(df52, df52$stockName=='AMZN')
Lamzn$startDayValue <-Lamzn$stockDayValue[1]
Lamzn$startDayDate <- Lamzn$Date[1]
Lamzn$finalDayValue <-Lamzn$stockDayValue[length(Lamzn$stockDayValue)]
Lamzn$finalDayDate <- Lamzn$Date[length(Lamzn$Date)]

Larwr <- subset(df52, df52$stockName=='ARWR')
Larwr$startDayValue <-Larwr$stockDayValue[1]
Larwr$startDayDate <- Larwr$Date[1]
Larwr$finalDayValue <-Larwr$stockDayValue[length(Larwr$stockDayValue)]
Larwr$finalDayDate <- Larwr$Date[length(Larwr$Date)]

Lc <- subset(df52, df52$stockName=='C')
Lc$startDayValue <-Lc$stockDayValue[1]
Lc$startDayDate <- Lc$Date[1]
Lc$finalDayValue <-Lc$stockDayValue[length(Lc$stockDayValue)]
Lc$finalDayDate <- Lc$Date[length(Lc$Date)]

Lcost <- subset(df52, df52$stockName=='COST')
Lcost$startDayValue <-Lcost$stockDayValue[1]
Lcost$startDayDate <- Lcost$Date[1]
Lcost$finalDayValue <-Lcost$stockDayValue[length(Lcost$stockDayValue)]
Lcost$finalDayDate <- Lcost$Date[length(Lcost$Date)]

Lcvx <- subset(df52, df52$stockName=='CVX')
Lcvx$startDayValue <-Lcvx$stockDayValue[1]
Lcvx$startDayDate <- Lcvx$Date[1]
Lcvx$finalDayValue <-Lcvx$stockDayValue[length(Lcvx$stockDayValue)]
Lcvx$finalDayDate <- Lcvx$Date[length(Lcvx$Date)]

Ldltr <- subset(df52, df52$stockName=='DLTR')
Ldltr$startDayValue <-Ldltr$stockDayValue[1]
Ldltr$startDayDate <- Ldltr$Date[1]
Ldltr$finalDayValue <-Ldltr$stockDayValue[length(Ldltr$stockDayValue)]
Ldltr$finalDayDate <- Ldltr$Date[length(Ldltr$Date)]

Lepd <- subset(df52, df52$stockName=='EPD')
Lepd$startDayValue <-Lepd$stockDayValue[1]
Lepd$startDayDate <- Lepd$Date[1]
Lepd$finalDayValue <-Lepd$stockDayValue[length(Lepd$stockDayValue)]
Lepd$finalDayDate <- Lepd$Date[length(Lepd$Date)]

Lf <- subset(df52, df52$stockName=='F')
Lf$startDayValue <-Lf$stockDayValue[1]
Lf$startDayDate <- Lf$Date[1]
Lf$finalDayValue <-Lf$stockDayValue[length(Lf$stockDayValue)]
Lf$finalDayDate <- Lf$Date[length(Lf$Date)]

Lffin <- subset(df52, df52$stockName=='FFIN')
Lffin$startDayValue <-Lffin$stockDayValue[1]
Lffin$startDayDate <- Lffin$Date[1]
Lffin$finalDayValue <-Lffin$stockDayValue[length(Lffin$stockDayValue)]
Lffin$finalDayDate <- Lffin$Date[length(Lffin$Date)]

Lftr <- subset(df52, df52$stockName=='FTR')
Lftr$startDayValue <-Lftr$stockDayValue[1]
Lftr$startDayDate <- Lftr$Date[1]
Lftr$finalDayValue <-Lftr$stockDayValue[length(Lftr$stockDayValue)]
Lftr$finalDayDate <- Lftr$Date[length(Lftr$Date)]

Lgoog <- subset(df52, df52$stockName=='GOOG')
Lgoog$startDayValue <-Lgoog$stockDayValue[1]
Lgoog$startDayDate <- Lgoog$Date[1]
Lgoog$finalDayValue <-Lgoog$stockDayValue[length(Lgoog$stockDayValue)]
Lgoog$finalDayDate <- Lgoog$Date[length(Lgoog$Date)]

Lhd <- subset(df52, df52$stockName=='HD')
Lhd$startDayValue <-Lhd$stockDayValue[1]
Lhd$startDayDate <- Lhd$Date[1]
Lhd$finalDayValue <-Lhd$stockDayValue[length(Lhd$stockDayValue)]
Lhd$finalDayDate <- Lhd$Date[length(Lhd$Date)]

Lhmc <- subset(df52, df52$stockName=='HMC')
Lhmc$startDayValue <-Lhmc$stockDayValue[1]
Lhmc$startDayDate <- Lhmc$Date[1]
Lhmc$finalDayValue <-Lhmc$stockDayValue[length(Lhmc$stockDayValue)]
Lhmc$finalDayDate <- Lhmc$Date[length(Lhmc$Date)]

Lhoft <- subset(df52, df52$stockName=='HOFT')
Lhoft$startDayValue <-Lhoft$stockDayValue[1]
Lhoft$startDayDate <- Lhoft$Date[1]
Lhoft$finalDayValue <-Lhoft$stockDayValue[length(Lhoft$stockDayValue)]
Lhoft$finalDayDate <- Lhoft$Date[length(Lhoft$Date)]

Lhrb <- subset(df52, df52$stockName=='HRB')
Lhrb$startDayValue <-Lhrb$stockDayValue[1]
Lhrb$startDayDate <- Lhrb$Date[1]
Lhrb$finalDayValue <-Lhrb$stockDayValue[length(Lhrb$stockDayValue)]
Lhrb$finalDayDate <- Lhrb$Date[length(Lhrb$Date)]

Lhst <- subset(df52, df52$stockName=='HST')
Lhst$startDayValue <-Lhst$stockDayValue[1]
Lhst$startDayDate <- Lhst$Date[1]
Lhst$finalDayValue <-Lhst$stockDayValue[length(Lhst$stockDayValue)]
Lhst$finalDayDate <- Lhst$Date[length(Lhst$Date)]

Lino <- subset(df52, df52$stockName=='INO')
Lino$startDayValue <-Lino$stockDayValue[1]
Lino$startDayDate <- Lino$Date[1]
Lino$finalDayValue <-Lino$stockDayValue[length(Lino$stockDayValue)]
Lino$finalDayDate <- Lino$Date[length(Lino$Date)]

Ljblu <- subset(df52, df52$stockName=='JBLU')
Ljblu$startDayValue <-Ljblu$stockDayValue[1]
Ljblu$startDayDate <- Ljblu$Date[1]
Ljblu$finalDayValue <-Ljblu$stockDayValue[length(Ljblu$stockDayValue)]
Ljblu$finalDayDate <- Ljblu$Date[length(Ljblu$Date)]

Ljnj <- subset(df52, df52$stockName=='JNJ')
Ljnj$startDayValue <-Ljnj$stockDayValue[1]
Ljnj$startDayDate <- Ljnj$Date[1]
Ljnj$finalDayValue <-Ljnj$stockDayValue[length(Ljnj$stockDayValue)]
Ljnj$finalDayDate <- Ljnj$Date[length(Ljnj$Date)]

Ljpm <- subset(df52, df52$stockName=='JPM')
Ljpm$startDayValue <-Ljpm$stockDayValue[1]
Ljpm$startDayDate <- Ljpm$Date[1]
Ljpm$finalDayValue <-Ljpm$stockDayValue[length(Ljpm$stockDayValue)]
Ljpm$finalDayDate <- Ljpm$Date[length(Ljpm$Date)]

Ljwn <- subset(df52, df52$stockName=='JWN')
Ljwn$startDayValue <-Ljwn$stockDayValue[1]
Ljwn$startDayDate <- Ljwn$Date[1]
Ljwn$finalDayValue <-Ljwn$stockDayValue[length(Ljwn$stockDayValue)]
Ljwn$finalDayDate <- Ljwn$Date[length(Ljwn$Date)]

Lkgji <- subset(df52, df52$stockName=='KGJI')
Lkgji$startDayValue <-Lkgji$stockDayValue[1]
Lkgji$startDayDate <- Lkgji$Date[1]
Lkgji$finalDayValue <-Lkgji$stockDayValue[length(Lkgji$stockDayValue)]
Lkgji$finalDayDate <- Lkgji$Date[length(Lkgji$Date)]

Lkss <- subset(df52, df52$stockName=='KSS')
Lkss$startDayValue <-Lkss$stockDayValue[1]
Lkss$startDayDate <- Lkss$Date[1]
Lkss$finalDayValue <-Lkss$stockDayValue[length(Lkss$stockDayValue)]
Lkss$finalDayDate <- Lkss$Date[length(Lkss$Date)]

Lluv <- subset(df52, df52$stockName=='LUV')
Lluv$startDayValue <-Lluv$stockDayValue[1]
Lluv$startDayDate <- Lluv$Date[1]
Lluv$finalDayValue <-Lluv$stockDayValue[length(Lluv$stockDayValue)]
Lluv$finalDayDate <- Lluv$Date[length(Lluv$Date)]

Lm <- subset(df52, df52$stockName=='M')
Lm$startDayValue <-Lm$stockDayValue[1]
Lm$startDayDate <- Lm$Date[1]
Lm$finalDayValue <-Lm$stockDayValue[length(Lm$stockDayValue)]
Lm$finalDayDate <- Lm$Date[length(Lm$Date)]

Lmgm <- subset(df52, df52$stockName=='MGM')
Lmgm$startDayValue <-Lmgm$stockDayValue[1]
Lmgm$startDayDate <- Lmgm$Date[1]
Lmgm$finalDayValue <-Lmgm$stockDayValue[length(Lmgm$stockDayValue)]
Lmgm$finalDayDate <- Lmgm$Date[length(Lmgm$Date)]

Lmsft <- subset(df52, df52$stockName=='MSFT')
Lmsft$startDayValue <-Lmsft$stockDayValue[1]
Lmsft$startDayDate <- Lmsft$Date[1]
Lmsft$finalDayValue <-Lmsft$stockDayValue[length(Lmsft$stockDayValue)]
Lmsft$finalDayDate <- Lmsft$Date[length(Lmsft$Date)]

Lnflx <- subset(df52, df52$stockName=='NFLX')
Lnflx$startDayValue <-Lnflx$stockDayValue[1]
Lnflx$startDayDate <- Lnflx$Date[1]
Lnflx$finalDayValue <-Lnflx$stockDayValue[length(Lnflx$stockDayValue)]
Lnflx$finalDayDate <- Lnflx$Date[length(Lnflx$Date)]

Lnke <- subset(df52, df52$stockName=='NKE')
Lnke$startDayValue <-Lnke$stockDayValue[1]
Lnke$startDayDate <- Lnke$Date[1]
Lnke$finalDayValue <-Lnke$stockDayValue[length(Lnke$stockDayValue)]
Lnke$finalDayDate <- Lnke$Date[length(Lnke$Date)]

Lnsany <- subset(df52, df52$stockName=='NSANY')
Lnsany$startDayValue <-Lnsany$stockDayValue[1]
Lnsany$startDayDate <- Lnsany$Date[1]
Lnsany$finalDayValue <-Lnsany$stockDayValue[length(Lnsany$stockDayValue)]
Lnsany$finalDayDate <- Lnsany$Date[length(Lnsany$Date)]

Lnus <- subset(df52, df52$stockName=='NUS')
Lnus$startDayValue <-Lnus$stockDayValue[1]
Lnus$startDayDate <- Lnus$Date[1]
Lnus$finalDayValue <-Lnus$stockDayValue[length(Lnus$stockDayValue)]
Lnus$finalDayDate <- Lnus$Date[length(Lnus$Date)]

Loncy <- subset(df52, df52$stockName=='ONCY')
Loncy$startDayValue <-Loncy$stockDayValue[1]
Loncy$startDayDate <- Loncy$Date[1]
Loncy$finalDayValue <-Loncy$stockDayValue[length(Loncy$stockDayValue)]
Loncy$finalDayDate <- Loncy$Date[length(Loncy$Date)]

Lpcg <- subset(df52, df52$stockName=='PCG')
Lpcg$startDayValue <-Lpcg$stockDayValue[1]
Lpcg$startDayDate <- Lpcg$Date[1]
Lpcg$finalDayValue <-Lpcg$stockDayValue[length(Lpcg$stockDayValue)]
Lpcg$finalDayDate <- Lpcg$Date[length(Lpcg$Date)]

Lrost <- subset(df52, df52$stockName=='ROST')
Lrost$startDayValue <-Lrost$stockDayValue[1]
Lrost$startDayDate <- Lrost$Date[1]
Lrost$finalDayValue <-Lrost$stockDayValue[length(Lrost$stockDayValue)]
Lrost$finalDayDate <- Lrost$Date[length(Lrost$Date)]

Lrrgb <- subset(df52, df52$stockName=='RRGB')
Lrrgb$startDayValue <-Lrrgb$stockDayValue[1]
Lrrgb$startDayDate <- Lrrgb$Date[1]
Lrrgb$finalDayValue <-Lrrgb$stockDayValue[length(Lrrgb$stockDayValue)]
Lrrgb$finalDayDate <- Lrrgb$Date[length(Lrrgb$Date)]

Ls <- subset(df52, df52$stockName=='S')
Ls$startDayValue <-Ls$stockDayValue[1]
Ls$startDayDate <- Ls$Date[1]
Ls$finalDayValue <-Ls$stockDayValue[length(Ls$stockDayValue)]
Ls$finalDayDate <- Ls$Date[length(Ls$Date)]

Lsig <- subset(df52, df52$stockName=='SIG')
Lsig$startDayValue <-Lsig$stockDayValue[1]
Lsig$startDayDate <- Lsig$Date[1]
Lsig$finalDayValue <-Lsig$stockDayValue[length(Lsig$stockDayValue)]
Lsig$finalDayDate <- Lsig$Date[length(Lsig$Date)]

Lt <- subset(df52, df52$stockName=='T')
Lt$startDayValue <-Lt$stockDayValue[1]
Lt$startDayDate <- Lt$Date[1]
Lt$finalDayValue <-Lt$stockDayValue[length(Lt$stockDayValue)]
Lt$finalDayDate <- Lt$Date[length(Lt$Date)]

Lteva <- subset(df52, df52$stockName=='TEVA')
Lteva$startDayValue <-Lteva$stockDayValue[1]
Lteva$startDayDate <- Lteva$Date[1]
Lteva$finalDayValue <-Lteva$stockDayValue[length(Lteva$stockDayValue)]
Lteva$finalDayDate <- Lteva$Date[length(Lteva$Date)]

Ltgt <- subset(df52, df52$stockName=='TGT')
Ltgt$startDayValue <-Ltgt$stockDayValue[1]
Ltgt$startDayDate <- Ltgt$Date[1]
Ltgt$finalDayValue <-Ltgt$stockDayValue[length(Ltgt$stockDayValue)]
Ltgt$finalDayDate <- Ltgt$Date[length(Ltgt$Date)]

Ltjx <- subset(df52, df52$stockName=='TJX')
Ltjx$startDayValue <-Ltjx$stockDayValue[1]
Ltjx$startDayDate <- Ltjx$Date[1]
Ltjx$finalDayValue <-Ltjx$stockDayValue[length(Ltjx$stockDayValue)]
Ltjx$finalDayDate <- Ltjx$Date[length(Ltjx$Date)]

Ltm <- subset(df52, df52$stockName=='TM')
Ltm$startDayValue <-Ltm$stockDayValue[1]
Ltm$startDayDate <- Ltm$Date[1]
Ltm$finalDayValue <-Ltm$stockDayValue[length(Ltm$stockDayValue)]
Ltm$finalDayDate <- Ltm$Date[length(Ltm$Date)]

Lubsi <- subset(df52, df52$stockName=='UBSI')
Lubsi$startDayValue <-Lubsi$stockDayValue[1]
Lubsi$startDayDate <- Lubsi$Date[1]
Lubsi$finalDayValue <-Lubsi$stockDayValue[length(Lubsi$stockDayValue)]
Lubsi$finalDayDate <- Lubsi$Date[length(Lubsi$Date)]

Lvz <- subset(df52, df52$stockName=='VZ')
Lvz$startDayValue <-Lvz$stockDayValue[1]
Lvz$startDayDate <- Lvz$Date[1]
Lvz$finalDayValue <-Lvz$stockDayValue[length(Lvz$stockDayValue)]
Lvz$finalDayDate <- Lvz$Date[length(Lvz$Date)]

Lwfc <- subset(df52, df52$stockName=='WFC')
Lwfc$startDayValue <-Lwfc$stockDayValue[1]
Lwfc$startDayDate <- Lwfc$Date[1]
Lwfc$finalDayValue <-Lwfc$stockDayValue[length(Lwfc$stockDayValue)]
Lwfc$finalDayDate <- Lwfc$Date[length(Lwfc$Date)]

Lwm <- subset(df52, df52$stockName=='WM')
Lwm$startDayValue <-Lwm$stockDayValue[1]
Lwm$startDayDate <- Lwm$Date[1]
Lwm$finalDayValue <-Lwm$stockDayValue[length(Lwm$stockDayValue)]
Lwm$finalDayDate <- Lwm$Date[length(Lwm$Date)]

Lwmt <- subset(df52, df52$stockName=='WMT')
Lwmt$startDayValue <-Lwmt$stockDayValue[1]
Lwmt$startDayDate <- Lwmt$Date[1]
Lwmt$finalDayValue <-Lwmt$stockDayValue[length(Lwmt$stockDayValue)]
Lwmt$finalDayDate <- Lwmt$Date[length(Lwmt$Date)]

Lwwe <- subset(df52, df52$stockName=='WWE')
Lwwe$startDayValue <-Lwwe$stockDayValue[1]
Lwwe$startDayDate <- Lwwe$Date[1]
Lwwe$finalDayValue <-Lwwe$stockDayValue[length(Lwwe$stockDayValue)]
Lwwe$finalDayDate <- Lwwe$Date[length(Lwwe$Date)]

Lxom <- subset(df52, df52$stockName=='XOM')
Lxom$startDayValue <-Lxom$stockDayValue[1]
Lxom$startDayDate <- Lxom$Date[1]
Lxom$finalDayValue <-Lxom$stockDayValue[length(Lxom$stockDayValue)]
Lxom$finalDayDate <- Lxom$Date[length(Lxom$Date)]
...
df52 <- rbind(Laal,Laap,Laddyy,Lamzn,Larwr,
              Lc,Lcost,Lcvx,Ldltr,Lepd,Lf,Lffin,Lftr,Lgoog,Lhd,
              Lhmc,Lhoft,Lhrb,Lhst,Lino,Ljblu,Ljnj,Ljpm,
              Ljwn,Lkgji,Lkss,Lluv,Lm,Lmgm,Lmsft,Lnflx,Lnke,
              Lnsany,Lnus,Loncy,Lpcg,Lrost,
              Lrrgb,Ls,Lsig,Lt,Lteva,Ltgt,Ltjx,Ltm,
              Lubsi,Lvz,Lwfc,Lwm,Lwmt,Lwwe,Lxom)
...
AAL3 <- merge(AAL1, countsAALtableDecr, by.x='cumulativeSumTodayGrtrThanLag7',
               by.y='cumulativeSumTodayGrtrThanLag7')
AAL3 <- AAL3[,c(3,7,9,8,10,2,4,13,14,15,1,17,16,18,5,6,11,12)]
...
AAL5 <- AAL4[,c(2:15,20,1,22,21,23,16:19)]
```
...
AAP3 <- merge(AAP1, countsAAPtableDecr, by.x='cumulativeSumTodayGrtrThanLag7',
               by.y='cumulativeSumTodayGrtrThanLag7')
AAP3 <- AAP3[,c(3,7,9,8,10,2,4,13,14,15,1,17,16,18,5,6,11,12)]
...
AAP5 <- AAP4[,c(2:15,20,1,22,21,23,16:19)]
```
...
ADDYY3 <- merge(ADDYY1, countsADDYYtableDecr, by.x='cumulativeSumTodayGrtrThanLag7',
               by.y='cumulativeSumTodayGrtrThanLag7')
ADDYY3 <- ADDYY3[,c(3,7,9,8,10,2,4,13,14,15,1,17,16,18,5,6,11,12)]
...
ADDYY5 <- ADDYY4[,c(2:15,20,1,22,21,23,16:19)]
```
...
AMZN3 <- merge(AMZN1, countsAMZNtableDecr, by.x='cumulativeSumTodayGrtrThanLag7',
               by.y='cumulativeSumTodayGrtrThanLag7')
AMZN3 <- AMZN3[,c(3,7,9,8,10,2,4,13,14,15,1,17,16,18,5,6,11,12)]
...
AMZN5 <- AMZN4[,c(2:15,20,1,22,21,23,16:19)]
```
...
ARWR3 <- merge(ARWR1, countsARWRtableDecr, by.x='cumulativeSumTodayGrtrThanLag7',
               by.y='cumulativeSumTodayGrtrThanLag7')
ARWR3 <- ARWR3[,c(3,7,9,8,10,2,4,13,14,15,1,17,16,18,5,6,11,12)]
...
ARWR5 <- ARWR4[,c(2:15,20,1,22,21,23,16:19)]
```
...
C3 <- merge(C1, countsCtableDecr, by.x='cumulativeSumTodayGrtrThanLag7',
               by.y='cumulativeSumTodayGrtrThanLag7')
C3 <- C3[,c(3,7,9,8,10,2,4,13,14,15,1,17,16,18,5,6,11,12)]
...
C5 <- C4[,c(2:15,20,1,22,21,23,16:19)]
```
...
COST3 <- merge(COST1, countsCOSTtableDecr, by.x='cumulativeSumTodayGrtrThanLag7',
               by.y='cumulativeSumTodayGrtrThanLag7')
COST3 <- COST3[,c(3,7,9,8,10,2,4,13,14,15,1,17,16,18,5,6,11,12)]
...
COST5 <- COST4[,c(2:15,20,1,22,21,23,16:19)]
```
...
CVX3 <- merge(CVX1, countsCVXtableDecr, by.x='cumulativeSumTodayGrtrThanLag7',
               by.y='cumulativeSumTodayGrtrThanLag7')
CVX3 <- CVX3[,c(3,7,9,8,10,2,4,13,14,15,1,17,16,18,5,6,11,12)]
...
CVX5 <- CVX4[,c(2:15,20,1,22,21,23,16:19)]
```
...
DLTR3 <- merge(DLTR1, countsDLTRtableDecr, by.x='cumulativeSumTodayGrtrThanLag7',
               by.y='cumulativeSumTodayGrtrThanLag7')
DLTR3 <- DLTR3[,c(3,7,9,8,10,2,4,13,14,15,1,17,16,18,5,6,11,12)]
...
DLTR5 <- DLTR4[,c(2:15,20,1,22,21,23,16:19)]
```
...
EPD3 <- merge(EPD1, countsEPDtableDecr, by.x='cumulativeSumTodayGrtrThanLag7',
               by.y='cumulativeSumTodayGrtrThanLag7')
EPD3 <- EPD3[,c(3,7,9,8,10,2,4,13,14,15,1,17,16,18,5,6,11,12)]
...
EPD5 <- EPD4[,c(2:15,20,1,22,21,23,16:19)]
```
...
F3 <- merge(F1, countsFtableDecr, by.x='cumulativeSumTodayGrtrThanLag7',
               by.y='cumulativeSumTodayGrtrThanLag7')
F3 <- F3[,c(3,7,9,8,10,2,4,13,14,15,1,17,16,18,5,6,11,12)]
...
F5 <- F4[,c(2:15,20,1,22,21,23,16:19)]
```
...
FFIN3 <- merge(FFIN1, countsFFINtableDecr, by.x='cumulativeSumTodayGrtrThanLag7',
               by.y='cumulativeSumTodayGrtrThanLag7')
FFIN3 <- FFIN3[,c(3,7,9,8,10,2,4,13,14,15,1,17,16,18,5,6,11,12)]
...
FFIN5 <- FFIN4[,c(2:15,20,1,22,21,23,16:19)]
```
...
FTR3 <- merge(FTR1, countsFTRtableDecr, by.x='cumulativeSumTodayGrtrThanLag7',
               by.y='cumulativeSumTodayGrtrThanLag7')
FTR3 <- FTR3[,c(3,7,9,8,10,2,4,13,14,15,1,17,16,18,5,6,11,12)]
...
FTR5 <- FTR4[,c(2:15,20,1,22,21,23,16:19)]
```
...
GOOG3 <- merge(GOOG1, countsGOOGtableDecr, by.x='cumulativeSumTodayGrtrThanLag7',
               by.y='cumulativeSumTodayGrtrThanLag7')
GOOG3 <- GOOG3[,c(3,7,9,8,10,2,4,13,14,15,1,17,16,18,5,6,11,12)]
...
GOOG5 <- GOOG4[,c(2:15,20,1,22,21,23,16:19)]
```
...
HD3 <- merge(HD1, countsHDtableDecr, by.x='cumulativeSumTodayGrtrThanLag7',
               by.y='cumulativeSumTodayGrtrThanLag7')
HD3 <- HD3[,c(3,7,9,8,10,2,4,13,14,15,1,17,16,18,5,6,11,12)]
...
HD5 <- HD4[,c(2:15,20,1,22,21,23,16:19)]
```
...
HMC3 <- merge(HMC1, countsHMCtableDecr, by.x='cumulativeSumTodayGrtrThanLag7',
               by.y='cumulativeSumTodayGrtrThanLag7')
HMC3 <- HMC3[,c(3,7,9,8,10,2,4,13,14,15,1,17,16,18,5,6,11,12)]
...
HMC5 <- HMC4[,c(2:15,20,1,22,21,23,16:19)]
```
...
HOFT3 <- merge(HOFT1, countsHOFTtableDecr, by.x='cumulativeSumTodayGrtrThanLag7',
               by.y='cumulativeSumTodayGrtrThanLag7')
HOFT3 <- HOFT3[,c(3,7,9,8,10,2,4,13,14,15,1,17,16,18,5,6,11,12)]
...
HOFT5 <- HOFT4[,c(2:15,20,1,22,21,23,16:19)]
```
...
HRB3 <- merge(HRB1, countsHRBtableDecr, by.x='cumulativeSumTodayGrtrThanLag7',
               by.y='cumulativeSumTodayGrtrThanLag7')
HRB3 <- HRB3[,c(3,7,9,8,10,2,4,13,14,15,1,17,16,18,5,6,11,12)]
...
HRB5 <- HRB4[,c(2:15,20,1,22,21,23,16:19)]
```
...
HST3 <- merge(HST1, countsHSTtableDecr, by.x='cumulativeSumTodayGrtrThanLag7',
               by.y='cumulativeSumTodayGrtrThanLag7')
HST3 <- HST3[,c(3,7,9,8,10,2,4,13,14,15,1,17,16,18,5,6,11,12)]
...
HST5 <- HST4[,c(2:15,20,1,22,21,23,16:19)]
```
...
INO3 <- merge(INO1, countsINOtableDecr, by.x='cumulativeSumTodayGrtrThanLag7',
               by.y='cumulativeSumTodayGrtrThanLag7')
INO3 <- INO3[,c(3,7,9,8,10,2,4,13,14,15,1,17,16,18,5,6,11,12)]
...
INO5 <- INO4[,c(2:15,20,1,22,21,23,16:19)]
```
...
JBLU3 <- merge(JBLU1, countsJBLUtableDecr, by.x='cumulativeSumTodayGrtrThanLag7',
               by.y='cumulativeSumTodayGrtrThanLag7')
JBLU3 <- JBLU3[,c(3,7,9,8,10,2,4,13,14,15,1,17,16,18,5,6,11,12)]
...
JBLU5 <- JBLU4[,c(2:15,20,1,22,21,23,16:19)]
```
...
JNJ3 <- merge(JNJ1, countsJNJtableDecr, by.x='cumulativeSumTodayGrtrThanLag7',
               by.y='cumulativeSumTodayGrtrThanLag7')
JNJ3 <- JNJ3[,c(3,7,9,8,10,2,4,13,14,15,1,17,16,18,5,6,11,12)]
...
JNJ5 <- JNJ4[,c(2:15,20,1,22,21,23,16:19)]
```
...
JPM3 <- merge(JPM1, countsJPMtableDecr, by.x='cumulativeSumTodayGrtrThanLag7',
               by.y='cumulativeSumTodayGrtrThanLag7')
JPM3 <- JPM3[,c(3,7,9,8,10,2,4,13,14,15,1,17,16,18,5,6,11,12)]
...
JPM5 <- JPM4[,c(2:15,20,1,22,21,23,16:19)]
```
...
JWN3 <- merge(JWN1, countsJWNtableDecr, by.x='cumulativeSumTodayGrtrThanLag7',
               by.y='cumulativeSumTodayGrtrThanLag7')
JWN3 <- JWN3[,c(3,7,9,8,10,2,4,13,14,15,1,17,16,18,5,6,11,12)]
...
JWN5 <- JWN4[,c(2:15,20,1,22,21,23,16:19)]
```
...
KGJI3 <- merge(KGJI1, countsKGJItableDecr, by.x='cumulativeSumTodayGrtrThanLag7',
               by.y='cumulativeSumTodayGrtrThanLag7')
KGJI3 <- KGJI3[,c(3,7,9,8,10,2,4,13,14,15,1,17,16,18,5,6,11,12)]
...
KGJI5 <- KGJI4[,c(2:15,20,1,22,21,23,16:19)]
```
...
KSS3 <- merge(KSS1, countsKSStableDecr, by.x='cumulativeSumTodayGrtrThanLag7',
               by.y='cumulativeSumTodayGrtrThanLag7')
KSS3 <- KSS3[,c(3,7,9,8,10,2,4,13,14,15,1,17,16,18,5,6,11,12)]
...
KSS5 <- KSS4[,c(2:15,20,1,22,21,23,16:19)]
```
...
LUV3 <- merge(LUV1, countsLUVtableDecr, by.x='cumulativeSumTodayGrtrThanLag7',
               by.y='cumulativeSumTodayGrtrThanLag7')
LUV3 <- LUV3[,c(3,7,9,8,10,2,4,13,14,15,1,17,16,18,5,6,11,12)]
...
LUV5 <- LUV4[,c(2:15,20,1,22,21,23,16:19)]
```
...
M3 <- merge(M1, countsMtableDecr, by.x='cumulativeSumTodayGrtrThanLag7',
               by.y='cumulativeSumTodayGrtrThanLag7')
M3 <- M3[,c(3,7,9,8,10,2,4,13,14,15,1,17,16,18,5,6,11,12)]
...
M5 <- M4[,c(2:15,20,1,22,21,23,16:19)]
```
...
MGM3 <- merge(MGM1, countsMGMtableDecr, by.x='cumulativeSumTodayGrtrThanLag7',
               by.y='cumulativeSumTodayGrtrThanLag7')
MGM3 <- MGM3[,c(3,7,9,8,10,2,4,13,14,15,1,17,16,18,5,6,11,12)]
...
MGM5 <- MGM4[,c(2:15,20,1,22,21,23,16:19)]
...
MSFT3 <- merge(MSFT1, countsMSFTtableDecr, by.x='cumulativeSumTodayGrtrThanLag7',
               by.y='cumulativeSumTodayGrtrThanLag7')
MSFT3 <- MSFT3[,c(3,7,9,8,10,2,4,13,14,15,1,17,16,18,5,6,11,12)]
...
MSFT5 <- MSFT4[,c(2:15,20,1,22,21,23,16:19)]
...
NFLX3 <- merge(NFLX1, countsNFLXtableDecr, by.x='cumulativeSumTodayGrtrThanLag7',
               by.y='cumulativeSumTodayGrtrThanLag7')
NFLX3 <- NFLX3[,c(3,7,9,8,10,2,4,13,14,15,1,17,16,18,5,6,11,12)]
...
NFLX5 <- NFLX4[,c(2:15,20,1,22,21,23,16:19)]
```
...
NKE3 <- merge(NKE1, countsNKEtableDecr, by.x='cumulativeSumTodayGrtrThanLag7',
               by.y='cumulativeSumTodayGrtrThanLag7')
NKE3 <- NKE3[,c(3,7,9,8,10,2,4,13,14,15,1,17,16,18,5,6,11,12)]
...
NKE5 <- NKE4[,c(2:15,20,1,22,21,23,16:19)]
```
...
NSANY3 <- merge(NSANY1, countsNSANYtableDecr, by.x='cumulativeSumTodayGrtrThanLag7',
               by.y='cumulativeSumTodayGrtrThanLag7')
NSANY3 <- NSANY3[,c(3,7,9,8,10,2,4,13,14,15,1,17,16,18,5,6,11,12)]
...
NSANY5 <- NSANY4[,c(2:15,20,1,22,21,23,16:19)]
```
...
NUS3 <- merge(NUS1, countsNUStableDecr, by.x='cumulativeSumTodayGrtrThanLag7',
               by.y='cumulativeSumTodayGrtrThanLag7')
NUS3 <- NUS3[,c(3,7,9,8,10,2,4,13,14,15,1,17,16,18,5,6,11,12)]
...
NUS5 <- NUS4[,c(2:15,20,1,22,21,23,16:19)]
```
...
ONCY3 <- merge(ONCY1, countsONCYtableDecr, by.x='cumulativeSumTodayGrtrThanLag7',
               by.y='cumulativeSumTodayGrtrThanLag7')
ONCY3 <- ONCY3[,c(3,7,9,8,10,2,4,13,14,15,1,17,16,18,5,6,11,12)]
...
ONCY5 <- ONCY4[,c(2:15,20,1,22,21,23,16:19)]
```
...
PCG3 <- merge(PCG1, countsPCGtableDecr, by.x='cumulativeSumTodayGrtrThanLag7',
               by.y='cumulativeSumTodayGrtrThanLag7')
PCG3 <- PCG3[,c(3,7,9,8,10,2,4,13,14,15,1,17,16,18,5,6,11,12)]
...
PCG5 <- PCG4[,c(2:15,20,1,22,21,23,16:19)]
```
...
ROST3 <- merge(ROST1, countsROSTtableDecr, by.x='cumulativeSumTodayGrtrThanLag7',
               by.y='cumulativeSumTodayGrtrThanLag7')
ROST3 <- ROST3[,c(3,7,9,8,10,2,4,13,14,15,1,17,16,18,5,6,11,12)]
...
ROST5 <- ROST4[,c(2:15,20,1,22,21,23,16:19)]
```
...
RRGB3 <- merge(RRGB1, countsRRGBtableDecr, by.x='cumulativeSumTodayGrtrThanLag7',
               by.y='cumulativeSumTodayGrtrThanLag7')
RRGB3 <- RRGB3[,c(3,7,9,8,10,2,4,13,14,15,1,17,16,18,5,6,11,12)]
...
RRGB5 <- RRGB4[,c(2:15,20,1,22,21,23,16:19)]
```
...
S3 <- merge(S1, countsStableDecr, by.x='cumulativeSumTodayGrtrThanLag7',
               by.y='cumulativeSumTodayGrtrThanLag7')
S3 <- S3[,c(3,7,9,8,10,2,4,13,14,15,1,17,16,18,5,6,11,12)]
...
S5 <- S4[,c(2:15,20,1,22,21,23,16:19)]
```
...
SIG3 <- merge(SIG1, countsSIGtableDecr, by.x='cumulativeSumTodayGrtrThanLag7',
               by.y='cumulativeSumTodayGrtrThanLag7')
SIG3 <- SIG3[,c(3,7,9,8,10,2,4,13,14,15,1,17,16,18,5,6,11,12)]
...
SIG5 <- SIG4[,c(2:15,20,1,22,21,23,16:19)]
```
...
T3 <- merge(T1, countsTtableDecr, by.x='cumulativeSumTodayGrtrThanLag7',
               by.y='cumulativeSumTodayGrtrThanLag7')
T3 <- T3[,c(3,7,9,8,10,2,4,13,14,15,1,17,16,18,5,6,11,12)]
...
T5 <- T4[,c(2:15,20,1,22,21,23,16:19)]
```
...
TEVA3 <- merge(TEVA1, countsTEVAtableDecr, by.x='cumulativeSumTodayGrtrThanLag7',
               by.y='cumulativeSumTodayGrtrThanLag7')
TEVA3 <- TEVA3[,c(3,7,9,8,10,2,4,13,14,15,1,17,16,18,5,6,11,12)]
...
TEVA5 <- TEVA4[,c(2:15,20,1,22,21,23,16:19)]
```
...
TGT3 <- merge(TGT1, countsTGTtableDecr, by.x='cumulativeSumTodayGrtrThanLag7',
               by.y='cumulativeSumTodayGrtrThanLag7')
TGT3 <- TGT3[,c(3,7,9,8,10,2,4,13,14,15,1,17,16,18,5,6,11,12)]
...
TGT5 <- TGT4[,c(2:15,20,1,22,21,23,16:19)]
```
...
TJX3 <- merge(TJX1, countsTJXtableDecr, by.x='cumulativeSumTodayGrtrThanLag7',
               by.y='cumulativeSumTodayGrtrThanLag7')
TJX3 <- TJX3[,c(3,7,9,8,10,2,4,13,14,15,1,17,16,18,5,6,11,12)]
...
TJX5 <- TJX4[,c(2:15,20,1,22,21,23,16:19)]
```
...
TM3 <- merge(TM1, countsTMtableDecr, by.x='cumulativeSumTodayGrtrThanLag7',
               by.y='cumulativeSumTodayGrtrThanLag7')
TM3 <- TM3[,c(3,7,9,8,10,2,4,13,14,15,1,17,16,18,5,6,11,12)]
...
TM5 <- TM4[,c(2:15,20,1,22,21,23,16:19)]
```
...
UBSI3 <- merge(UBSI1, countsUBSItableDecr, by.x='cumulativeSumTodayGrtrThanLag7',
               by.y='cumulativeSumTodayGrtrThanLag7')
UBSI3 <- UBSI3[,c(3,7,9,8,10,2,4,13,14,15,1,17,16,18,5,6,11,12)]
...
UBSI5 <- UBSI4[,c(2:15,20,1,22,21,23,16:19)]
```
...
VZ3 <- merge(VZ1, countsVZtableDecr, by.x='cumulativeSumTodayGrtrThanLag7',
               by.y='cumulativeSumTodayGrtrThanLag7')
VZ3 <- VZ3[,c(3,7,9,8,10,2,4,13,14,15,1,17,16,18,5,6,11,12)]
...
VZ5 <- VZ4[,c(2:15,20,1,22,21,23,16:19)]
```
...
WFC3 <- merge(WFC1, countsWFCtableDecr, by.x='cumulativeSumTodayGrtrThanLag7',
               by.y='cumulativeSumTodayGrtrThanLag7')
WFC3 <- WFC3[,c(3,7,9,8,10,2,4,13,14,15,1,17,16,18,5,6,11,12)]
...
WFC5 <- WFC4[,c(2:15,20,1,22,21,23,16:19)]
```
...
WM3 <- merge(WM1, countsWMtableDecr, by.x='cumulativeSumTodayGrtrThanLag7',
               by.y='cumulativeSumTodayGrtrThanLag7')
WM3 <- WM3[,c(3,7,9,8,10,2,4,13,14,15,1,17,16,18,5,6,11,12)]
...
WM5 <- WM4[,c(2:15,20,1,22,21,23,16:19)]
```
...
WMT3 <- merge(WMT1, countsWMTtableDecr, by.x='cumulativeSumTodayGrtrThanLag7',
               by.y='cumulativeSumTodayGrtrThanLag7')
WMT3 <- WMT3[,c(3,7,9,8,10,2,4,13,14,15,1,17,16,18,5,6,11,12)]
...
WMT5 <- WMT4[,c(2:15,20,1,22,21,23,16:19)]
```
...
WWE3 <- merge(WWE1, countsWWEtableDecr, by.x='cumulativeSumTodayGrtrThanLag7',
               by.y='cumulativeSumTodayGrtrThanLag7')
WWE3 <- WWE3[,c(3,7,9,8,10,2,4,13,14,15,1,17,16,18,5,6,11,12)]
...
WWE5 <- WWE4[,c(2:15,20,1,22,21,23,16:19)]
```
...
XOM3 <- merge(XOM1, countsXOMtableDecr, by.x='cumulativeSumTodayGrtrThanLag7',
               by.y='cumulativeSumTodayGrtrThanLag7')
XOM3 <- XOM3[,c(3,7,9,8,10,2,4,13,14,15,1,17,16,18,5,6,11,12)]
...
XOM5 <- XOM4[,c(2:15,20,1,22,21,23,16:19)]
```
...
colnames(STOCK_COUNTS_52)[1] <- 'stockName'
STOCK_COUNTS_52 <- STOCK_COUNTS_52[,c(1:3,8,4,9,22,23,6,5,7,24,25,10:21)]

stockCompleteSet <- merge(comp,STOCK_COUNTS_52, by.x='stockName', by.y='stockName')
```
...
STOCK_COUNTS_52 <- STOCK_COUNTS_52[with(STOCK_COUNTS_52,order(stockName,Date)),]

write.csv(stockCompleteSet, 
          paste(paste(paste(paste('stocksFullDateREQS',
                      date,sep='_'),today,sep='_'),
                      lagN,sep='_'),
                      'csv', sep='.'), 
          row.names=FALSE)

write.csv(STOCK_COUNTS_52,
          paste(paste(paste(paste('STOCK_COUNTS_52',date,sep='_'),today,sep='_'),
                      lagN,sep='_'),'csv',sep='.')
          , row.names=FALSE)
```
...
Combine the 52 and excluded data so that there is a table of all stocks, but some stocks have different dates.
```{r}
all65 <- rbind(STOCK_COUNTS_52,inCompREQ_stocks)
All65 <- all65[with(all65,order(stockName, Date)),]
row.names(All65) <- NULL
```

Write this file out to csv.
```{r}

write.csv(All65,paste(paste(paste(paste('ALL_65_stocks_countGroups', 
                            date, sep='_'),
                            today, sep='_'),
                            lagN,sep='_'),
                            'csv', sep='.'),
...
!@#$% This section uses the link in the beginning to the [tidytext](https://www.tidytextmining.com/dtm.html) library instructions for text mining in R with other text mining R libraries. 

Its really too bad this part of the tutorial didn't work, it was great in the tutorial, and could have made some immediate use of it, but the google and yahoo stock support for corpus of documents ended more than a year ago.
```{r,eval=FALSE}
library(tm)
library(tm.plugin.webmining)
library(purrr)
...
# symbol <- as.factor(paste(c("MSFT", "AAPL", "GOOG", "AMZN", "FB", "TWTR", "IBM", "YHOO", "NFLX")))
...
# reuters <- WebCorpus(ReutersNewsSource("businessNews"))#default
#has to use the string: http://feeds.reuters.com/reuters/ before the tab name in 
# http://www.reuters.com/tools/rss
#these produce lists that are empty
# reuterst <- WebCorpus(ReutersNewsSource("http://feeds.reuters.com/reuters/techNews"))
# reutersm <- WebCorpus(ReutersNewsSource("http://feeds.reuters.com/reuters/marketsNews"))
# reutersf <- WebCorpus(ReutersNewsSource("http://feeds.reuters.com/reuters/businessNews/finance"))
```
Note that the google and yahoo don't work, so this tutorial cannot be used for those news sources. Reuters might still be an option. The BusinessNews is the only corpus available as a list. 
...
inaug_tf_idf
```
This following demo doesn't work like the first set commented out. Chapter 5 has some problems or 
```{r,eval=FALSE}
library(tidyr)

# year_term_counts <- inaug_td %>%
#   extract(document, "year", "(\\d+)", convert = TRUE) %>% 
# # Error in UseMethod("extract", x) : 
# #   no applicable method for 'extract' applied to an object of class "c('tbl_df', 'tbl', 'data.frame')"
#   # PlainText() doesn't work
#   # the 'year' isn't recognized as a function type for 'extractor'.
#   # str_extract() doesn't work for the year 
#   complete(year, term, fill = list(count = 0)) %>%
#   # Error in UseMethod("complete_") : 
#   # no applicable method for 'complete_' applied to an object of class "c('PlainTextDocument', 'TextDocument')"
#   group_by(year) %>%
#   mutate(year_total = sum(count))

ytc <- inaug_td$document
ytc1 <- strsplit(ytc,split='-')

ytc2 <- lapply(ytc1,'[',1)
ytc3 <- as.numeric(paste(ytc2))
inaug_td$year <- ytc3

inaug_td2 <- inaug_td %>% group_by(year,term,count) %>% 
  count(term) %>% ungroup()

year_term_counts <- inaug_td2[,1:3]

exp <- c('-','\"','(',')',',','.',':',';')

year_term_counts2 <- year_term_counts %>% 
  filter(!term %in% exp)

yt3 <- year_term_counts2 %>% mutate(Year=as.factor(paste(year_term_counts2$year))) %>%
  group_by(Year) %>% mutate(year_total = sum(count))
head(yt3, 30)
```
The above was slightly modified from the tutorial for the plot that follows.
