...
trainingSet$NextDay2PercentIncrease <- round(trainingSet$NextDay2PercentIncrease,0)
testingSet$NextDay2PercentIncrease <- round(testingSet$NextDay2PercentIncrease,0)
```
...
predDF_round <- data.frame(predRF_round, NextDay2PercentIncrease=testingSet$NextDay2PercentIncrease)
predDF_round

sum <- sum(predRF_round==testingSet$NextDay2PercentIncrease) 
...
```{r}
knnMod_round <- train(NextDay2PercentIncrease ~ .,
                method='knn', preProcess=c('center','scale'),
                tuneLength=10, trControl=trainControl(method='cv'), data=trainingSet)
```
...
```{r, error=FALSE, message=FALSE, warning=FALSE}
rpartMod_round <- train(NextDay2PercentIncrease ~ ., method='rpart', tuneLength=9, data=trainingSet) 

```

```{r, error=FALSE, message=FALSE, warning=FALSE}
glmMod_round <- train(NextDay2PercentIncrease ~ ., 
                method='glm', data=trainingSet) 
```
...
```{r}
sumKNN_round <- sum(predKNN_round==testingSet$NextDay2PercentIncrease)
sumRPart_round <- sum(predRPART_round==testingSet$NextDay2PercentIncrease)
```

```{r}
sumGLM_round <- sum(predGLM_round==testingSet$NextDay2PercentIncrease)
...
colnames(predDF2_round) <- c('RandomForest_round','KNN_round','Rpart_round','GLM_round','NextDay2PercentIncrease')
...
results_round <- as.factor(results_round)
results_round <- t(data.frame(results_round))
colnames(results_round) <- c('RandomForest_round','KNN_round','Rpart_round','GLM_round','NextDay2PercentIncrease')
Results_round <- rbind(predDF2_round, results_round) 
Results_round
...
Results_round$GLM_round <-
  as.numeric(paste(Results_round$GLM_round))
Results_round$NextDay2PercentIncrease <-
  as.numeric(paste(Results_round$NextDay2PercentIncrease))
...
sMost <- sum(Results_round$mostVoted==Results_round$NextDay2PercentIncrease)
lMost <- length(Results_round$NextDay2PercentIncrease)-1
accMost <- sMost/lMost
accMost
Results_round
```
