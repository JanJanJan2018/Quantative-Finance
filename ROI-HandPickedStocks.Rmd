...
#name the rows by the date column
row.names(remaining36) <- as.factor(remaining36$Date)

#order by date
remaining36 <- remaining36[order(remaining36$Date),]

#remove the date column
remaining36 <- remaining36[,-1]
```

```{r}
write.csv(remaining36,'remaining36.csv', row.names=TRUE)
```

Add all the lags to each stock from a 7 day lag, 30 day lag,...180 day lag. Then add the ratio of each day's stock value to the lag. Then get the counts of cumulative days the ratio increased and decreased. Then add the max, median, and 3rd quantile stats of those cumulative day counts for increased and decreased.
```{r}
